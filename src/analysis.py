...
class AreaResult:
    ...
    def draw_plot(
        ...
        return ax
    def write_results(self, outdir, plot=True):
        ...
        fig, ax = plt.subplots()
        ax.boxplot(summary.strain, )
        for strain in set(self.df.strain):
            if plot:
                fig, ax = plt.subplots()
                area.draw_plot(ax, fit=True, strains=[strain])
                plt.savefig(Path(outdir, f"{strain}.png"))
                plt.close(fig)


area = AreaResult("../analysis/sample_data/area.csv", "../analysis/sample_data/id.csv")
area.fit_all(3, 6)
area.write_results("../analysis/sample_output/")
