...
class ImageProcessor:
    def __init__(self, filepath, args):
        ...
        self.image_debugger.debug_image(self.rgb, f"Raw: {self.filepath}")
        ...
        self.a = a
        self.image_debugger.debug_image(a, f"Green-Red channel (a in Lab)")
        self.b = b
        self.image_debugger.debug_image(b, f"Blue-Yellow channel (b in Lab)")
        self.logger.debug("Convert RGB to HSV and split")
        h, s, v = cv2.split(cv2.cvtColor(self.rgb, cv2.COLOR_RGB2HSV))
        self.s = s
        self.image_debugger.debug_image(s, "Saturation (S in HSV)")
        self.v = v
        self.image_debugger.debug_image(v, "Value (V in HSV)")
    ...
    def get_circle_mask(self, plates):
        ...
        self.image_debugger.debug_image(circle_mask, "Circle mask")
        return circle_mask
    ...
    def get_image_mask(self):
        ...
        self.image_debugger.debug_image(s_thresh, f"Saturation threshold: {args.saturation}")
        ...
        self.image_debugger.debug_image(v_thresh, f"Value threshold: {args.value}")
        ...
        self.image_debugger.debug_image(colour_mask, f"Value or Saturation joined (colour mask)")
        ...
        self.image_debugger.debug_image(a_thresh, f"Green-Red (a in Lab) threshold: {args.green_red}")
        ...
        self.image_debugger.debug_image(b_thresh, f"Blue-Yellow (b in Lab) threshold: {args.blue_yellow}")

        self.logger.debug("Join the a and b channels to create a green mask")
        green_mask = cv2.bitwise_and(a_thresh, b_thresh)
        self.image_debugger.debug_image(green_mask, f"a and b join (green mask)")
        ...
        self.image_debugger.debug_image(image_mask, f"Colour and green joined (image mask)")
    ...
    def get_mask(self, plates):
        ...
        self.image_debugger.debug_image(mask, "Colour mask and blue ring mask joined")

        self.logger.debug("Remove small objects in the mask")
        clean_mask = remove_small_objects(img_as_bool(mask), args.remove)
        self.image_debugger.debug_image(clean_mask, "Cleaned mask (removed small objects)")

        self.logger.debug("Remove small holes in the mask")
        filled_mask = remove_small_holes(clean_mask, args.fill)
        self.image_debugger.debug_image(clean_mask, "Filled mask (removed small holes)")
    ...
    def get_area(self):
        ...
        layout = Layout(args, self.b, self.image_debugger)
        plates = layout.get_plates()
        ...
        for p in plates:
            self.logger.debug(f"Processing plate {p.plate_id}")
            for j, c in enumerate(p.circles):
        ...
        if args.overlay:
            ...
            self.image_debugger.debug_image(overlay, "Overlay (unlabeled)")
            for p in plates:
                self.logger.debug(f"Annotate overlay with plate ID: {p.plate_id}")
            ...
            self.image_debugger.debug_image(overlay, "Overlay (annotated)")
