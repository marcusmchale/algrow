...
class ImageProcessor:
    ...
    def find_circles(self, image, param2=30):
        args = self.args
        self.logger.debug("Find the rings in image")
    ...
    def find_plates(self):
        args = self.args
        self.logger.debug("Blur blue-yellow greyscale image")
        ...
        for param2 in ph_list:
            ...
            if circles is None:
                self.logger.debug(f"No circles found with param2 = {param2}")
                continue
            ...
            if circles.shape[0] < 48:
                ...
                continue
            self.logger.debug(f"found {str(circles.shape[0])} circles with param2 = {param2}, proceeding to clustering")
            if args.image_debug:
            ...
            self.logger.debug("Cut the dendrogram and select clusters containing 6 centre points only")
            ...
            self.logger.debug(f"Found {len(target_clusters)} clusters with param2 = {param2}")
            if len(target_clusters) < 8:
                continue
            elif len(target_clusters) > 8:
                raise ImageContentException("More than 8 clusters of 6 circles found")
            else:
                self.logger.info(f"{self.filepath}: param2 in HoughCircles = {param2}")
